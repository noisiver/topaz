-----------------------------------------
-- ID: 13975
-- Equip: Evoker's Bracers
-- 25% chance to absorb dmg taken into MP of element of avatar currently out
-----------------------------------------
    ...
    for i, element in pairs(pets) do
        if target:getPetID() == element[1] and target:getPetID() ~= nil then
            ...
            if hands == 13975 then -- Evoker's Bracers
                target:addMod(absorb, 25)
                target:setLocalVar("Absorb" .. absorb, 25)
            end
        end
    end
end