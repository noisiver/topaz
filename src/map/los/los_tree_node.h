...
{
public:
    LosTreeNode(Triangle* elements, BoundingBox* boundingBoxes, int* elementNexts, int* elementIndices, int indexStart, int indexEnd, int splitsLeft,
                float boxSizeThreshold, size_t elementsThreshold, bool normalSplit = false);

    ~LosTreeNode();

    LosTreeNodeStats GetStats(int* elementNexts, Triangle* elements);

    std::optional<Vector3D> DoesRayCollide(BoundingBox& bounds, Vector3D& rayOrigin, Vector3D& rayVector, int* elementNexts, Triangle* elements) const;
