   ...
   if not mob:isFollowingPath() then
        if mob:getLocalVar('isPaused') ~= 0 then
            ...
            if newReverse == 0 then
                mob:pathThrough(pathNodes, tpz.path.flag.COORDS)
            else
                mob:pathThrough(pathNodes, bit.bor(tpz.path.flag.COORDS, tpz.path.flag.REVERSE))
            end
        else
            ...
            mob:pathThrough(rotations, tpz.path.flag.COORDS)
...
function onMobSpawn(mob)
    ...
    mob:setLocalVar('isPaused', 0)
    mob:setLocalVar('mobPath', 1)
    mob:pathThrough(pathStart, tpz.path.flag.COORDS)
    onPath(mob)
end
