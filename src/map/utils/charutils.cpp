...
namespace charutils
{
    ...
    ************************************************************************/
        ...
        void SaveJobChangeGear(CCharEntity* PChar)
    {
        ...
        Sql_Query(SqlHandle,Query, PChar->id, PChar->GetMJob(), main, sub, ranged, ammo, head, body, hands, legs, feet, neck, waist, ear1, ear2, ring1, ring2, back);
    }
    ...
    void LoadJobChangeGear(CCharEntity* PChar)
    {
        ...
        const char* Query = "SELECT main, sub, ranged, ammo, head, body, hands, legs, feet, neck, waist, ear1, ear2, ring1, ring2, back FROM char_equip_saved "
                            "AS equip WHERE charid = %u AND jobid = %u;";

        if (Sql_Query(SqlHandle, Query, PChar->id, PChar->GetMJob()) == SQL_SUCCESS && Sql_NumRows(SqlHandle) != 0 && Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            for (uint8 equipSlot = SLOT_MAIN; equipSlot <= SLOT_BACK; equipSlot++)
            {
                uint16 itemId = Sql_GetUIntData(SqlHandle, equipSlot);
    ...
    ************************************************************************/
    EMobDifficulty CheckMob(uint8 charlvl, uint8 moblvl)
    {
        ...
        if (baseExp >= 370) return EMobDifficulty::IncrediblyTough;
        if (baseExp >= 220) return EMobDifficulty::VeryTough;
        ...
        if (baseExp >= 50) return EMobDifficulty::EasyPrey;
