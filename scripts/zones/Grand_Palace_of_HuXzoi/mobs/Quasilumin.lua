...
function shouldMove(mob, progress)
    return not mob:isFollowingPath() and mob:getStatus() == tpz.status.NORMAL and progress ~= EscortProgress.COMPLETE
end

function closeDoor(mob)
...
function onMobRoam(mob)
    ...
    if progress == EscortProgress.NONE then
        ...
        mob:pathThrough(escorts[mob:getLocalVar("escort")].path[point], tpz.path.flag.WALK)
    end
	...
	local recommencetext = mob:getLocalVar("recommencetext")
    for i, entity in pairs(entities) do
        if entity:getAggressive() == 1 and mob:getID() ~= entity:getID() and entity:isSpawned() and entity:getCurrentAction() == tpz.action.ROAMING then
            ...
            if progress ~= EscortProgress.PAUSED then
				...
				mob:setLocalVar("recommencetext", 0)
            end
        end
    end
    ...
    if opened_door ~= 0 then
        ...
        if mob:checkDistance(npc) > 15 then
            npc:setAnimation(tpz.animation.CLOSE_DOOR)
            mob:setLocalVar("opened_door", 0)
        elseif npc:getAnimation() ~= tpz.animation.OPEN_DOOR then
...
function onPath(mob)
    ...
    if data ~= nil and shouldMove(mob, progress) then
