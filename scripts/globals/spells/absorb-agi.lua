...
function onSpellCast(caster, target, spell)
    ...
    if (resist >= 0.5) then
        ...
        caster:delStatusEffectSilent(tpz.effect.AGI_BOOST)
        target:delStatusEffectSilent(tpz.effect.AGI_DOWN)
	    caster:addStatusEffect(tpz.effect.AGI_BOOST, power, tick, duration) -- caster gains ACC
	    target:addStatusEffect(tpz.effect.AGI_DOWN, power, tick, duration)   -- target loses ACC
        caster:delStatusEffectSilent(tpz.effect.NETHER_VOID)
    else
        spell:setMsg(tpz.msg.basic.MAGIC_RESIST)
    end
    return tpz.effect.AGI_BOOST
end
