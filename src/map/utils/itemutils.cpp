...
namespace itemutils
{
    ...
    ************************************************************************/
    ...
    void LoadDropList()
    {
        int32 ret = Sql_Query(SqlHandle, "SELECT dropId, dropType, itemId, rarity, groupId, groupRate FROM mob_droplist WHERE dropid < %u;", MAX_DROPID);
        ...
        if( ret != SQL_ERROR && Sql_NumRows(SqlHandle) != 0)
        {
            while(Sql_NextRow(SqlHandle) == SQL_SUCCESS)
            {
                ...
                uint8 DropType = (uint8)Sql_GetIntData(SqlHandle, 1);
                uint16 ItemID = (uint16)Sql_GetIntData(SqlHandle, 2);
                uint16 Rarity = (uint16)Sql_GetIntData(SqlHandle, 3);
                ...
                if (DropType == DROP_GROUPED)
                {
                    ...
                    while (GroupId >= dropList->Groups.size())
                        dropList->Groups.emplace_back();

                    if (dropList->Groups[GroupId].Items.empty())
                        dropList->Groups[GroupId].GroupRate = Rarity;

                    dropList->Groups[GroupId].Items.emplace_back(DropType, ItemID, GroupRate);
                }
                else if (DropType == DROP_NORMAL)
                {
                    dropList->Items.emplace_back(DropType, ItemID, Rarity);
                }
                else
                {
                    dropList->Items.emplace_back(DropType, ItemID, 255);
                }
            }
        }
    }
