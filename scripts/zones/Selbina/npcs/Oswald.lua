...
-- !pos 48 -15 9 248
-- TODO: The Real Gift unique reward
-----------------------------------
...
function onEventFinish(player, csid, option)
    ...
    elseif csid == 37 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, tpz.quest.id.otherAreas.UNDER_THE_SEA, {item = 13335, fame_area = SELBINA, fame = 300, xp = 4500, title = tpz.title.LIL_CUPID, var = "underTheSeaVar"}) then
    ...
    elseif csid == 72 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, tpz.quest.id.otherAreas.THE_GIFT, {item = 16497, fame_area = SELBINA, fame = 350, xp = 4500, title = tpz.title.SAVIOR_OF_LOVE}) then
        player:confirmTrade()
    elseif csid == 73 and option == 50 then
        player:addQuest(OTHER_AREAS_LOG, tpz.quest.id.otherAreas.THE_REAL_GIFT)
    elseif csid == 75 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, tpz.quest.id.otherAreas.THE_REAL_GIFT, {item = 17385, fame_area = SELBINA, fame = 500, xp = 7500, title = tpz.title.THE_LOVE_DOCTOR}) then
        player:confirmTrade()
    end
end
