...
template <typename F> int splitArraySort(int* arr, int start, int end, F&& splitFunc)
{
    while (start < end)
    {
...
LosTreeNode::LosTreeNode(Triangle* elements, BoundingBox* boundingBoxes, int* elementNexts, int* elementIndices, int indexStart, int indexEnd, int splitsLeft,
                         float boxSizeThreshold, size_t elementsThreshold, bool normalSplit)
{
    ...
    {
        left = new LosTreeNode(elements, boundingBoxes, elementNexts, elementIndices, indexStart, indexSplit - 1, splitsLeft - 1, boxSizeThreshold,
                               elementsThreshold);
        minY = left->minY;
        maxY = left->maxY;
    }
    ...
    {
        right =
            new LosTreeNode(elements, boundingBoxes, elementNexts, elementIndices, indexSplit, indexEnd, splitsLeft - 1, boxSizeThreshold, elementsThreshold);
        if (right->minY < minY)
        {
            minY = right->minY;
        }
...
LosTreeNodeStats LosTreeNode::GetStats(int* elementNexts, Triangle* elements)
{
    ...
    if (headElementIdx != -1)
    {
        ...
        stats.maxAxis =
            std::max(std::max(stats.boundingBox.getAxisSize(Axis::X), stats.boundingBox.getAxisSize(Axis::Y)), stats.boundingBox.getAxisSize(Axis::Z));
