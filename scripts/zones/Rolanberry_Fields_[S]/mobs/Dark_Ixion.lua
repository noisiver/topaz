...
function onMobFight(mob, target)
    ...
    if Ashed == 0 then -- Failed to hit with stygian ash
        ...
        if RNG == 1 then
            mob:pathTo(-36, 0, -822) -- Grauberg
        elseif RNG == 2 then
            mob:pathTo(275, 24, 439) -- Karugo-Narugo
        end
        mob:setLocalVar("RunAwayWait", 60)
        if BattleTime >= RunAwayWait and RunAwayWait > 0 then
            local ixion = 0
            DespawnMob(mob:getID())
            if RNG == 1 then
                ixion = GetMobByID(17142112) -- Grauberg
            elseif RNG == 2  then
                ixion = GetMobByID(17170649) -- Karugo-Narugo
            end
            ixion:spawn()
            mob:setLocalVar("RunAwayWait", 0)
        end
    ...
    if StartTrampling == 1 then
        if TrackingTarget == 0 then
            ...
            elseif RNG == 2 then
                mob:pathTo(275, 24, 439) -- Karugo-Narugo
            elseif RNG == 3 then
                mob:pathTo(-627, -23, -211) -- West
            elseif RNG == 4 then
                mob:pathTo(-332, -20, -614) -- SouthWest
            elseif RNG == 5 then
                mob:pathTo(306, -32, -307) -- East
            elseif RNG == 6 then
                mob:pathTo(-406, 0, 196) -- NorthWest
            end
        end
