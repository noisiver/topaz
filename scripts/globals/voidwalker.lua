...
local function searchEmptyPos(zoneId)
    local maxPos = table.getn(tpz.voidwalker.pos[zoneId])
...
local function setRandomPos(zoneId, mobId)
    local mob = GetMobByID(mobId)
    if not mob or not tpz.voidwalker.pos[zoneId] then
        return
    end
...
local mixinByMobName =
{
    ...
    ['Jyeshtha'] = function(mob)
        ...
        if mob:getLocalVar("MOBSKILL_USE") == 1 and not mob:hasStatusEffect(tpz.effect.MIGHTY_STRIKES) then
    ...
    ['Erebus'] = function(mob)
        ...
        if mob:hasStatusEffect(tpz.effect.BLOOD_WEAPON) and not mob:hasStatusEffect(tpz.effect.HUNDRED_FISTS) then
...
tpz.voidwalker.onHealing = function(player)
    ...
    elseif mobNearest.distance <= 4 then
        ...
        if mobNearest.keyItem ~= tpz.keyItem.CLEAR_ABYSSITE and mobNearest.keyItem ~= tpz.keyItem.COLORFUL_ABYSSITE then
