    ...
    if (target:hasStatusEffect(tpz.effect.MND_DOWN) or caster:getStatusEffect(tpz.effect.MND_BOOST)) then
        ...
        if (resist < 0.5) then
			...
			if caster:hasStatusEffect(tpz.effect.NETHER_VOID) then
				caster:addStatusEffect(tpz.effect.MND_BOOST, ABSORB_SPELL_AMOUNT*resist*((100+(caster:getMod(tpz.mod.AUGMENTS_ABSORB)))/100) * NetherVoidBonus, ABSORB_SPELL_TICK, ABSORB_SPELL_AMOUNT*ABSORB_SPELL_TICK) -- caster gains MND
				target:addStatusEffect(tpz.effect.MND_DOWN, ABSORB_SPELL_AMOUNT*resist*((100+(caster:getMod(tpz.mod.AUGMENTS_ABSORB)))/100) * NetherVoidBonus, ABSORB_SPELL_TICK, ABSORB_SPELL_AMOUNT*ABSORB_SPELL_TICK)    -- target loses MND
