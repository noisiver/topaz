...
bool CLatentEffectContainer::ProcessLatentEffect(CLatentEffect& latentEffect)
{
    ...
    case LATENT_MAINJOB:
        ...
        break;
    case LATENT_SIGNET_EXP_BONUS:
        expression = m_POwner->loc.zone->GetRegionID() < 28 && m_POwner->GetMLevel() > 54 && m_POwner->GetMLevel() < 74;
        break;
    case LATENT_SANCTION_EXP_BONUS:
        expression =  m_POwner->loc.zone->GetRegionID() >= 28 && m_POwner->loc.zone->GetRegionID() <= 32 && m_POwner->GetMLevel() > 54 && m_POwner->GetMLevel() < 74;
        break;
    case LATENT_WEAPON_DRAWN_HP_UNDER:
        expression = m_POwner->health.hp < latentEffect.GetConditionsValue() && m_POwner->animation == ANIMATION_ATTACK;
        break;
    case LATENT_SIGIL_EXP_BONUS:
        expression = m_POwner->loc.zone->GetRegionID() >= 33 && m_POwner->loc.zone->GetRegionID() <= 40 && m_POwner->GetMLevel() > 54 && m_POwner->GetMLevel() < 74;
        break;
    case LATENT_MP_UNDER_VISIBLE_GEAR:
